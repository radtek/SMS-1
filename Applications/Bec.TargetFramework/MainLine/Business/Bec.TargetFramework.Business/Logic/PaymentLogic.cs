...
namespace Bec.TargetFramework.Business.Logic
{
    ...
    public class PaymentLogic : LogicBase, IPaymentLogic
    {
        private readonly PaymentSettings m_PaymentSettings;

        public PaymentLogic(ILogger logger, ICacheProvider cacheProvider, PaymentSettings paymentSettings) : base(logger, cacheProvider)
        {
        ...
        private ErrorCodeDTO GetErrorCodeDto(ErrorCodeTypeIDEnum typeEnum, ErrorCodeCategoryIDEnum categoryEnum, string code)
        {
                ...
                false))
            {
                ...
                var errorCodes =  scope.DbContext.ErrorCodes.Where(
                        s =>
                            s.ErrorCodeCategoryID.HasValue &&
                            s.ErrorCodeCategoryID.Value.Equals(categoryId)
                            && s.ErrorCodeTypeID.HasValue && s.ErrorCodeTypeID.Value.Equals(typeId)
                            && s.ErrorCode1.Equals(code)).ToList();

                if (errorCodes.Count > 0)
                    dto = ErrorCodeConverter.ToDto(errorCodes.First());
                else
                    dto =ErrorCodeConverter.ToDto( scope.DbContext.ErrorCodes.Single(s => s.ErrorCode1.Equals("5999")));
            }
        ...
        public TransactionOrderPaymentDTO GetTheSuccessfulOrderPaymentForTransactionOrder(Guid transactionOrderId)
        {
                ...
                )
            {
                ...
                var statusType = LogicHelper.GetStatusType(scope,
                    StatusTypeEnum.TransactionOrderProcessLog.GetStringValue(),
                    TransactionOrderStatusEnum.Successful.GetStringValue());

                var logs = scope.DbContext.TransactionOrderProcessLogs.Where(s => s.TransactionOrderID.Equals(transactionOrderId)
                    && s.StatusTypeID.Equals(statusType.StatusTypeID) && s.TransactionOrderPaymentID.HasValue
                    && s.StatusTypeValueID.Equals(statusType.StatusTypeValueID)).ToList();
                ...
                if (logs.Count > 0)
                {
                    ...
                    dto =
                        TransactionOrderPaymentConverter.ToDto(
                            scope.DbContext.TransactionOrderPayments.Single(
                                s => s.TransactionOrderPaymentID.Equals(paymentId.Value)));
                } 
            }
        ...
        public bool DoesASuccessfulOrderPaymentExistForTransactionOrder(Guid transactionOrderId)
        {
                ...
                )
            {
                ...
                exist = scope.DbContext.TransactionOrderProcessLogs.Any(s => s.TransactionOrderID.Equals(transactionOrderId)
                    && s.StatusTypeID.Equals(statusType.StatusTypeID)
                    && s.StatusTypeValueID.Equals(statusType.StatusTypeValueID));
            }
        ...
        public TransactionOrderPaymentDTO ProcessPaymentTransaction(VUserAccountOrganisationDTO clientUaoDto,TransactionOrderDTO transactionOrderDto, OrderRequestDTO request)
        {
            ...
            X509Certificate2Collection col = store.Certificates.Find(X509FindType.FindBySerialNumber, m_PaymentSettings.KeySerialNumber, false);

            NetworkCredential nc = new NetworkCredential(m_PaymentSettings.KeyID,m_PaymentSettings.Password);
            o.Credentials = nc;
            ...
            if (col.Count == 1)
            {
                ...
                try
                {
                    ...
                    if (clientUaoDto != null)
                    {
                        Billing b = new Billing();

                        b.Zip = clientUaoDto.PostalCode;
                        b.Country = clientUaoDto.CountryCode;
                        b.Address1 = clientUaoDto.Line1;
                        b.Address2 = clientUaoDto.Line2;
                        b.City = clientUaoDto.City;
                        b.Email = clientUaoDto.Email;
                        b.State = clientUaoDto.County;
