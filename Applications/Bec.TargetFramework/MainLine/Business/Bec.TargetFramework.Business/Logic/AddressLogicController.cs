...
namespace Bec.TargetFramework.Business.Logic
{
    ...
    public class AddressLogicController : LogicBase
    {
        ...
        public async Task<GoogleGeoCodeResponse> GeoCodePostcodeAsync(string postCode)
        {
            Ensure.That(postCode).IsNotNullOrEmpty();

            WebRequest request = WebRequest.Create(string.Format("http://maps.googleapis.com/maps/api/geocode/json?address={0}&sensor=false", postCode.Trim().Replace(" ", "")));

            request.Credentials = CredentialCache.DefaultCredentials;

            var result = JsonSerializer.DeserializeResponse<GoogleGeoCodeResponse>(await request.GetResponseAsync());
